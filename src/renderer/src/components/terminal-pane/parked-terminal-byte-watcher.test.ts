...
describe('startParkedTerminalByteWatcher', () => {
  ...
  describe('with main side-effect authority on', () => {
    ...
    // NOTE (standalone parking extraction): #7214's copy of this suite also
    // has an emitViaMainTrackerFacts helper + a byte-vs-fact parity case that
    // replay the same bytes through shared/terminal-output-side-effects (the
    // main-process tracker). That module is fact-mode machinery outside this
    // byte-mode extraction, so the parity case returns with PR #7214.
