...
describe('FloatingTerminalPanel close behavior', () => {
  ...
  it('keeps the empty floating workspace focused after Cmd+W closes the last tab', async () => {
    ...
    expect(mocks.closeTab).toHaveBeenCalledWith('tab-1', { captureRecentlyClosed: false })
    expect(panelElement.focus).toHaveBeenCalledWith({ preventScroll: true })
  })
  ...
  it('keeps the panel open when the explicit close action leaves another tab', async () => {
    ...
    expect(mocks.closeTab).toHaveBeenCalledWith('tab-2', { captureRecentlyClosed: false })
    expect(onOpenChange).not.toHaveBeenCalled()
  })
  ...
  it('reads the current tab list for bulk close actions', async () => {
    ...
    expect(mocks.closeTab).toHaveBeenCalledWith('new-left', { captureRecentlyClosed: false })
    expect(mocks.closeTab).toHaveBeenCalledWith('new-right', { captureRecentlyClosed: false })
    expect(mocks.closeTab).not.toHaveBeenCalledWith('old-left')

    mocks.closeTab.mockClear()
    ;(tabBar.props.onCloseToRight as (tabId: string) => void)('new-left')
    expect(mocks.closeTab).toHaveBeenCalledWith('new-keep', { captureRecentlyClosed: false })
    expect(mocks.closeTab).toHaveBeenCalledWith('new-right', { captureRecentlyClosed: false })
  ...
  it('closes tabs to the right using visible tab order', async () => {
    ...
    expect(mocks.closeTab).toHaveBeenCalledWith('tab-a', { captureRecentlyClosed: false })
    expect(mocks.closeTab).toHaveBeenCalledWith('tab-b', { captureRecentlyClosed: false })
